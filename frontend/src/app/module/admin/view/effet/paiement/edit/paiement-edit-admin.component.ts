...
export class PaiementEditAdminComponent extends AbstractEditController<PaiementDto, PaiementCriteria, PaiementAdminService>   implements OnInit {
    ...
    public prepareEdit() {
        this.item.dateEmission = this.paiementService.format(this.item.dateEmission);
        this.item.dateEcheance = this.paiementService.format(this.item.dateEcheance);
        this.item.dateEncaissement = this.paiementService.format(this.item.dateEncaissement);
    }
   ...
   public async openCreateBeneficiaire(beneficiaire: string) {
        const isPermistted = await this.roleService.isPermitted('Beneficiaire', 'edit');
        if (isPermistted) {
             this.beneficiaire = new BeneficiaireDto();
